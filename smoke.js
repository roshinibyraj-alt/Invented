...
(async () => {
  // Test 1: GREEN candle → BUY UP. Up ask dips to 0.30 then bounces.
  // Fill when bid ≤ limit (bid = ask - 0.01 = 0.29).
  // 3 fills: limit 0.40, 0.35, 0.30 (0.29 ≤ each).
  {
    ...
    console.log('\n--- RACE: GREEN candle → UP dip to 0.30 → bounce ---');
  ...
  // Test 2: RED candle → BUY DOWN. Down ask dips to 0.30.
  {
    ...
    await engine.discoverWindow(FIRST_WINDOW + WINDOW);
    await engine.pollClob(); engine.evaluate();
    await engine.pollClob(); engine.evaluate();   // DOWN ask 0.30 → bid 0.29 → fills
    await engine.pollClob(); engine.evaluate();
    const filled = engine.pendingOrders.filter(o => o.status === 'FILLED');
    console.log('\n--- RACE RED: candle RED → DOWN dip to 0.30 ---');
    filled.forEach(o => console.log('  ✅', o.outcome, 'limit', o.limitPrice, '@ fill', o.fillPrice));
    console.log('  filled:', filled.length, '| bank:', engine.bankroll.toFixed(2));
  ...
  // Test 5: ALL 6 FILLS — deep dip to 0.10
  {
    ...
    const engine = mkEngine(script);
    engine.candle.lastColor = 'GREEN'; // → BUY UP
    await engine.discoverWindow(FIRST_WINDOW);
    await engine.discoverWindow(FIRST_WINDOW + WINDOW);
    await engine.pollClob(); engine.evaluate();
    await engine.pollClob(); engine.evaluate();   // UP ask 0.10 → bid 0.09 → all 6 limits fill
    await engine.pollClob(); engine.evaluate();
    const filled = engine.pendingOrders.filter(o => o.status === 'FILLED');
    console.log('\n--- ALL FILLS: GREEN → UP deep dip to 0.10 ---');
    filled.forEach(o => console.log('  ✅', o.outcome, 'limit', o.limitPrice, '@ fill', o.fillPrice));
    console.log('  filled:', filled.length, '| bank:', engine.bankroll.toFixed(2));
    if (filled.length !== 6) failures.push('ALL FILLS: expected 6 fills, got ' + filled.length);
  }

  // Test 6: PARTIAL FILL — UP dip to 0.22, only top 4 limits fill
  {
    ...
    await engine.discoverWindow(FIRST_WINDOW + WINDOW);
    await engine.pollClob(); engine.evaluate();
    await engine.pollClob(); engine.evaluate();   // UP ask 0.22 → bid 0.21 → limits ≤ 0.21 fill
    await engine.pollClob(); engine.evaluate();
    const filled = engine.pendingOrders.filter(o => o.status === 'FILLED');
    const pending = engine.pendingOrders.filter(o => o.status === 'PENDING');
    console.log('\n--- PARTIAL: GREEN → UP dip to 0.22 ---');
    filled.forEach(o => console.log('  ✅', o.outcome, 'limit', o.limitPrice, '@ fill', o.fillPrice));
