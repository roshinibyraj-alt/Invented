...
const cricketBot = require('./cricket-bot');
...
app.get('/api/cricket/status', (_, res) => {
  try { res.json(cricketBot.getStatus()); } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
app.post('/api/cricket/pause', (_, res) => {
  try { res.json(cricketBot.pauseTrading()); } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
app.post('/api/cricket/resume', (_, res) => {
  try { res.json(cricketBot.resumeTrading()); } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
app.post('/api/cricket/set-mode', (req, res) => {
  const { live } = req.body || {};
  if (typeof live !== 'boolean') return res.status(400).json({ ok: false, error: 'Missing boolean "live" field' });
  try { res.json(cricketBot.setMode(live)); } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
<body>
  ...
  <div class="section" id="cricket-section">
    <div class="section-hdr">🏏 Cricket Trailing Ladder — Nepal Moneyline (single event)</div>
    <div class="toolbar">
      <button id="cricket-pause-btn" class="pause">⏸️ Pause</button>
      <button id="cricket-resume-btn" class="resume">▶️ Resume</button>
      <button id="cricket-live-btn" class="live-toggle">🔴 Go LIVE</button>
      <span id="cricket-mode-badge" class="mode-badge mode-dry">DEMO</span>
    </div>
    <div class="toolbar-status" id="cricket-toolbar-status"></div>
    <div class="window-strip" id="cricket-status-strip"></div>
    <div class="stats-row" id="cricket-stats-row"></div>
    <div class="ladder-grid" id="cricket-ladder-grid"></div>
  </div>
  ...
  <div class="bottom-grid">
    <div class="section">
      <div class="section-hdr">Cricket Trade Log</div>
      <div class="tbl-wrap">
        <table class="tbl">
          <thead><tr><th>Time</th><th>Rung</th><th>Reason</th><th>Price</th><th>Shares</th><th>P&amp;L</th></tr></thead>
          <tbody id="cricket-trade-body"><tr><td colspan="6" class="empty">Loading…</td></tr></tbody>
        </table>
      </div>
    </div>
    <div class="section">
      <div class="section-hdr">Cricket Log</div>
      <div class="logs-wrap" id="cricket-logs"></div>
    </div>
  </div>
...
<script>
  ...
  $('cricket-pause-btn').onclick = () => fetch('/api/cricket/pause', { method: 'POST' }).then(() => cricketFlash('Paused'));
  $('cricket-resume-btn').onclick = () => fetch('/api/cricket/resume', { method: 'POST' }).then(() => cricketFlash('Resumed'));
  $('cricket-live-btn').onclick = () => {
    const wantLive = !$('cricket-live-btn').classList.contains('is-live');
    if (wantLive && !confirm('Switch cricket ladder to LIVE mode? This will place REAL resting limit orders with REAL money.')) return;
    fetch('/api/cricket/set-mode', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ live: wantLive }) })
      .then(() => cricketFlash(wantLive ? 'Switched to LIVE' : 'Switched to DEMO'));
  };
  function cricketFlash(msg) { $('cricket-toolbar-status').textContent = msg; setTimeout(() => { $('cricket-toolbar-status').textContent = ''; }, 3000); }
  ...
  socket.on('cricketState', (s) => {
    const badge = $('cricket-mode-badge');
    badge.textContent = s.dryRun ? 'DEMO' : 'LIVE';
    badge.className = 'mode-badge ' + (s.dryRun ? 'mode-dry' : 'mode-live');
    $('cricket-live-btn').classList.toggle('is-live', !s.dryRun);
    $('cricket-live-btn').textContent = s.dryRun ? '🔴 Go LIVE' : '🟡 Go DEMO';

    const m = s.market || {};
    const statusTxt = {
      discovering: 'Finding the Nepal moneyline market…',
      'awaiting-price': 'Market found, waiting for a live price to anchor the grid…',
      trading: 'Trading',
      resolved: 'Match resolved — ' + (m.resolvedWinner || 'unknown') + ' — ladder is done',
      error: 'Could not lock onto the market — check the log below',
    }[m.status] || m.status || '—';
    $('cricket-status-strip').innerHTML = statusTxt + (m.marketQuestion ? (' | ' + m.marketQuestion) : '') +
      ' | ask ' + (m.ask != null ? m.ask.toFixed(2) : '—') + ' / bid ' + (m.bid != null ? m.bid.toFixed(2) : '—');

    const cstats = [
      ['Bankroll', '$' + s.bankroll.toFixed(2), ''],
      ['Mark Value', '$' + s.markValue.toFixed(2), pClass(s.totalPnl)],
      ['Total P&amp;L', sgn(s.totalPnl), pClass(s.totalPnl)],
      ['Realized P&amp;L', sgn(s.realizedPnl), pClass(s.realizedPnl)],
      ['Wins / Losses', s.wins + ' / ' + s.losses, ''],
      ['Rebates Earned', '$' + s.rebatesEarned.toFixed(4), 'pnl-pos'],
    ];
    $('cricket-stats-row').innerHTML = cstats.map(([label, val, cls]) =>
      '<div class="stat"><div class="stat-label">' + label + '</div><div class="stat-val ' + cls + '">' + val + '</div></div>'
    ).join('');

    const cgrid = $('cricket-ladder-grid');
    if (!s.rungs || !s.rungs.length) {
      cgrid.innerHTML = '<div class="empty">Loading…</div>';
    } else {
      const rows = s.rungs.map(r => {
        let rowCls = 'empty', stateTxt = 'idle';
        if (r.position && !r.position.closed) {
          ...
          rowCls = 'empty';
          stateTxt = 'maxed out — no further re-entries';
      ...
      cgrid.innerHTML = '<div class="ladder-card has-pos"><div class="ladder-hdr"><div class="ladder-sym up">Nepal</div></div><div class="ladder-body">' + rows + '</div></div>';
    }

    const ctb = $('cricket-trade-body');
    if (s.trades && s.trades.length > 0) {
      ctb.innerHTML = s.trades.map(t => {
        const pnlStr = (t.profit !== undefined) ? sgn(t.profit) : '—';
        const pnlCls = (t.profit !== undefined) ? pClass(t.profit) : '';
        const sideColor = t.reason === 'ENTRY' ? '#ffd740' : (t.reason === 'RESOLUTION' ? '#00d4ff' : '#00e676');
        return '<tr><td>' + t.time + '</td><td style="color:' + sideColor + '">' + (t.rung || '—') + '</td>' +
          '<td>' + (t.reason || '—') + '</td>' +
          '<td>' + (t.price || 0).toFixed(3) + '</td>' +
      ...
      ctb.innerHTML = '<tr><td colspan="6" class="empty">No trades yet</td></tr>';
    }

    const clogEl = $('cricket-logs');
    if (s.logs && s.logs.length > 0) {
      clogEl.innerHTML = s.logs.map(l => {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  // Cricket ladder is independent of the crypto bot — a failure here (e.g. market
  // discovery, auth) should not take down the crypto ladders, so it's non-fatal.
  cricketBot.init(PK, emit, slog).catch(e => {
    console.error('❌ Cricket ladder init failed:', e.message);
  });
});
