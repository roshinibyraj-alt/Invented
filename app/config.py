...
Single engine -- delayed expensive-side entry, continuous 0.15
trailing stop, one flat-size flip, TP redemption:

  1. Entry: from window open, wait ENTRY_WAIT_SECONDS (90s). At that
     point, look at both sides' MID price ((bid+ask)/2) once and buy
     whichever is MORE expensive ("the expensive side", the side the
     market currently favors) -- but ONLY if that side's mid is inside
     the entry zone [ENTRY_ZONE_LOW, ENTRY_ZONE_HIGH] (0.35-0.80). If
     it's outside the zone at the 90s mark, no trade is taken this
     window. This is a single check at t=90s, not a rearmed watch --
     the zone/expensive-side gating applies to this initial entry only.
     No new entry (initial or flip) fires at all once
     NO_TRADE_AFTER_SECONDS (270s) of the window has elapsed. The
     actual fill is a real taker buy priced off ask depth (see
     Engine._realistic_fill_price), which can differ from the mid that
     triggered it by the spread.
  2. Trailing stop: continuous, not stepped, and driven off MID price
     ...
     high_water_mark - TRAIL_DISTANCE (0.15), rounded to the cent
  ...
  4. Flip on stop hit: when the trailing stop is hit (whether the
     ...
     buy against ask depth (unless NO_TRADE_AFTER_SECONDS has already
     elapsed, in which case the flip is skipped and the window just
     ends flat). The flip position gets the exact same continuous 0.15
     trailing-stop treatment. Capped at MAX_FLIPS_PER_WINDOW (1): once
     that one flip has been used, a further stop-out just ends the
     window flat -- no re-entry, no second flip.
...
# ---- Delayed expensive-side entry / continuous trailing stop / flip engine ----
ENTRY_WAIT_SECONDS = 90.0         # wait this long after window open before checking entry
NO_TRADE_AFTER_SECONDS = 270.0    # no new entries or flips once this much of the window has elapsed
ENTRY_ZONE_LOW = 0.35             # entry zone floor -- initial entry only
ENTRY_ZONE_HIGH = 0.80            # entry zone ceiling -- initial entry only
TP_PRICE = 0.99                   # take-profit level -- hit = redeemed at $1.00, fee-free
TRAIL_DISTANCE = 0.15             # continuous trailing stop distance from high-water mark
