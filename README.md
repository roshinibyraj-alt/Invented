...
single ladder-breakout strategy with pullback limit entries.
...
2. **Arm**: after the cold start, watch both sides' mid-price every
   tick. Whichever side's mid-price first reaches 0.75 becomes the
...
3. **Ladder**: every time the armed side's mid-price climbs through the
   next threshold (0.75 → 0.85 → 0.95), place one new resting limit BUY
   order (maker), priced 0.10 below that threshold:
...
- `LADDER_ARM_DELAY_SECONDS`, `LADDER_THRESHOLDS`, `LADDER_OFFSET`, `LADDER_SL_PRICE`, `LADDER_TP_PRICE`, `LADDER_SHARES_PER_RUNG`
...
- If price shoots straight through multiple thresholds in one tick
  (e.g. 0.70 → 0.96), every rung up to and including the one just
  crossed gets placed in that same tick.
